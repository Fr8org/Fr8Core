...
$commandText = "DECLARE @kill varchar(8000) = ''; SELECT @kill = @kill + 'kill ' + CONVERT(varchar(5), spid) + ';' FROM master..sysprocesses WHERE dbid = db_id('MyDB') EXEC(@kill); "
...
$commandText = "CREATE DATABASE $($databaseName)Staging AS COPY OF $($serverName).$($databaseName)"
Write-Host $commandText
