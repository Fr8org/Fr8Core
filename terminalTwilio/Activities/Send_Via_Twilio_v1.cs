...
namespace terminalTwilio.Activities
{
    public class Send_Via_Twilio_v1 : TerminalActivity<Send_Via_Twilio_v1.ActivityUi>
    {
        ...
        {
            var smsNumber = ActivityUI.SmsNumber.TextValue;
            if (string.IsNullOrWhiteSpace(smsNumber))
            {
                throw new ApplicationException("Can't send SMS to empty number");
            }
            var smsBody = ActivityUI.SmsBody.TextValue;
            if (string.IsNullOrWhiteSpace(smsBody))
            {
                throw new ApplicationException("Can't send SMS with empty text");
            }
            smsNumber = GeneralizePhoneNumber(smsNumber);
