...
namespace terminalGoogleTests.Integration
{
    public class ActivityConfigurator
    {
        ...
        public async Task<PlanDTO> CreateNewPlan()
        {
            ...
            var planDTO =
                await
                    baseHubIntTest.HttpPostAsync<PlanEmptyDTO, PlanDTO>(baseHubIntTest.GetHubApiBaseUrl() + "plans",
                        newPlan);
        ...
        public async Task<ActivityDTO> AddAndConfigure_QueryDocuSign(PlanDTO plan, int ordering)
        {
            ...
            var apmActivityTemplate =
                activityTemplates.SelectMany(a => a.Activities).Single(a => a.Name == "Query_DocuSign");
            ...
            queryDocuSignActivity =
                await
                    baseHubIntTest.HttpPostAsync<ActivityDTO, ActivityDTO>(
                        baseHubIntTest.GetHubApiBaseUrl() + "activities/save", queryDocuSignActivity);
        ...
        public async Task<ActivityDTO> AddAndConfigure_SaveToGoogleSheet(PlanDTO plan, int ordering,
            string manufestTypeToAssert, string crateDescriptionLabelToAssert, string newSpeadsheetName, Guid authToken)
        {
            ...
            saveToGoogleActivity.ActivityDTO =
                await
                    baseHubIntTest.HttpPostAsync<ActivityDTO, ActivityDTO>(
                        baseHubIntTest.GetHubApiBaseUrl() + "activities/save", saveToGoogleActivity.ActivityDTO);
            ...
            using (var crateStorage = baseHubIntTest.Crate.GetUpdatableStorage(saveToGoogleActivity.ActivityDTO))
            {
                ...
                var crateChoose =
                    contrls.Where(x => x.Type == ControlTypes.CrateChooser && x.Name == "UpstreamCrateChooser")
                        .SingleOrDefault() as CrateChooser;
                crateStorage.Remove<StandardConfigurationControlsCM>();
        ...
        public async Task<AuthorizationTokenDTO> DocuSign_AuthToken(int terminalId)
        {
            ...
            var token =
                await
                    baseHubIntTest.HttpPostAsync<CredentialsDTO, JObject>(
                        baseHubIntTest.GetHubApiBaseUrl() + "authentication/token", creds);
            Assert.AreNotEqual(token["error"].ToString(),
                "Unable to authenticate in DocuSign service, invalid login name or password.",
        ...
        public async Task<AuthorizationTokenDTO> Google_AuthToken(int terminalId)
        {
            ...
            var authTokens =
                await
                    baseHubIntTest.HttpGetAsync<List<ManageAuthToken_Terminal>>(baseHubIntTest.GetHubApiBaseUrl() +
                                                                                "ManageAuthToken");
