...
using StructureMap;
using terminalSalesforce.Controllers;
...
namespace terminalIntegrationTests
{
    ...
    {
        [Test]
        public void TerminalSalesForce_DiscoverTerminals_ShouldReturnDataInCorrectFormat()
        {
            //Arrange
            TerminalController _terminalController = ObjectFactory.GetInstance<TerminalController>();
            //Act
            var result = _terminalController.Get();
            ...
            //Assert
            Assert.IsNotNull(result, "The terminal discovery has failed for Terminal SalesForce");
