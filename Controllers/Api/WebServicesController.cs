    ...
    public class WebServicesController : ApiController
    {
        ...
        public IHttpActionResult GetActivities(ActivityCategory[] categories)
        {
            ...
            using (var uow = ObjectFactory.GetInstance<IUnitOfWork>())
            {
                ...
                var unknownService = uow.WebServiceRepository.GetQuery().FirstOrDefault(x => x.Name == UknownWebServiceName);
                ...
                {
                    WebServiceIconPath = x.WebService != null ? x.WebService.IconPath : (unknownService != null ? unknownService.IconPath : null),
                    WebServiceName = x.WebService != null ? x.WebService.Name : string.Empty,
                    //If we have activities with the same name but different versions then we take only one with the highest version
