...
$iisAppPoolName = "dockyard-pool"
$iisAppPoolDotNetVersion = "v4.0"
$iisAppName = "dockyard-test"
$directoryPath = "$env:APPVEYOR_BUILD_FOLDER" 
$port = 8080 

write-host "Website directory path: $($directoryPath)"
...
{
	#create the site
	$iisApp = New-Item $iisAppName -bindings @{protocol="http";bindingInformation=":$($port):"} -physicalPath $directoryPath
	$iisApp | Set-ItemProperty -Name "applicationPool" -Value $iisAppPoolName

	write-host "IIS website created at localhost, port number $($port)"
}
