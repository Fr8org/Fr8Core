...
{
    [TestFixture]
    [Category("Archive_DocuSign_Template")]
    public class Archive_DocuSign_Template_v1_Tests : BaseTest
    {
        [Test]
        public void ActivityIsValid_WhenIsNotConfigured_ReturnsFalse()
        {
            var target = new Archive_DocuSign_Template_v1();
            string errorMessage;
        ...
        {
            var target = new Archive_DocuSign_Template_v1(DocuSignActivityFixtureData.DocuSignManagerWithoutTemplates());
        ...
        {
            var target = new Archive_DocuSign_Template_v1(DocuSignActivityFixtureData.DocuSignManagerWithTemplates());
            var activityDO = FixtureData.TestActivity1();
            activityDO = target.Configure(activityDO, FixtureData.AuthToken_TerminalIntegration()).Result;
            SetTextBoxText(activityDO);
            string errorMessage;
        ...
        [Test]
        public void ActivityIsValid_WhenFileNameIsNotSpecified_ReturnsFalse()
        {
            var target = new Archive_DocuSign_Template_v1(DocuSignActivityFixtureData.DocuSignManagerWithTemplates());
            var activityDO = FixtureData.TestActivity1();
            activityDO = target.Configure(activityDO, FixtureData.AuthToken_TerminalIntegration()).Result;
            ...
            var result = target.ActivityIsValid(activityDO, out errorMessage);
            Assert.IsFalse(result);
            Assert.AreEqual("File name is not specified", errorMessage);
        }

        [Test]
        public void ActivityIsValid_WhenAllFieldsAreSpecified_ReturnsTrue()
        {
            var target = new Archive_DocuSign_Template_v1(DocuSignActivityFixtureData.DocuSignManagerWithTemplates());
            var activityDO = FixtureData.TestActivity1();
            activityDO = target.Configure(activityDO, FixtureData.AuthToken_TerminalIntegration()).Result;
        ...
        private void SetTextBoxText(ActivityDO activity)
        {
            using (var crateStorage = new CrateManager().GetUpdatableStorage(activity))
            {
                var configControls = crateStorage.FirstCrate<StandardConfigurationControlsCM>(c => true).Content;
                var fileNameTextBox = configControls.Controls.OfType<TextBox>().FirstOrDefault();
                fileNameTextBox.Value = "Some text";
            }
        }
    }
}
