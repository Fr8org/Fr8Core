    ...
    class Extract_Data_From_Envelopes_v1_EndToEnd_Tests : BaseHubIntegrationTest
    {
        ...
        public async Task Extract_Data_From_Envelopes_EndToEnd()
        {
            for (int i = 0; i < 3; i ++)
            {
                ...
                if (CheckForAuthenticationError(_solution))
                {
                    // Expired token. Run test again
                    continue;
                }
        ...
        private static bool CheckForAuthenticationError(ActivityDTO activityDto)
        {
            if (activityDto.CrateStorage != null)
            {
                var storage = CrateStorageSerializer.Default.ConvertFromDto(activityDto.CrateStorage);
                var auth = storage.CrateContentsOfType<StandardAuthenticationCM>().FirstOrDefault();
            ...
            if (activityDto.ChildrenActivities != null)
            {
                foreach (var child in activityDto.ChildrenActivities)
                {
                    if (CheckForAuthenticationError(child))
                    {
                        return true;
                    }
                }
