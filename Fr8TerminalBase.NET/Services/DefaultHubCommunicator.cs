...
namespace Fr8.TerminalBase.Services
{
    public class DefaultHubCommunicator : IHubCommunicator
    {
        ...
        public async Task<PlanNoChildrenDTO> LoadPlan(PlanFullDTO planContents)
        {
            var uri = new Uri($"{GetHubUrlWithApiVersion()}/plans/load");
            return await _restfulServiceClient.PostAsync<PlanFullDTO, PlanNoChildrenDTO>(uri, planContents);
        }
